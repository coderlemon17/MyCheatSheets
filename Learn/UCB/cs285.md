...
- The key idea for this lecture is to find *better estimate* of $Q(s,a)$.
...
- Due to the stochasticity of RL, a better **"reward to go"** can be an **average** over possible trajectories; and this also bring less variance than single trajectory sample. $Var(\bar X)=\frac{1}{n}Var(x_i)$
  - <img src="./pic/image-20201121130053471.png" alt="image-20201121130053471" style="zoom:20%;" />
...
- Monte Carlo policy evaluation:
  ...
  - MC evaluation with function approximation (Neural Network)
    ...
    - It's not good as $\sum\limits_i\sum\limits_{t'=t}^Tr(s_{i,t'},a_{i,t'})$, but still good estimation.
...
- Improve the *target* used in MC evaluation: 
  ...
  - The ideal target is a slightly incorrect because of $\bar V_\phi^\pi(s_{i,{t+1}})$ , so at initial state it's better to use MC target. But it has a lower variance (especially useful when the environment is very stochastic). 
  - This's referred to a `bootstrapped` estimate.
...
- PG with discount factors:
  - <img src=".\pic\image-20210227174540037.png" alt="image-20210227174540037" style="zoom: 50%;" />
  ...
  - The critic <img src=".\pic\image-20210227174754103.png" alt="image-20210227174754103" style="zoom: 70%;" /> is actually **equals to** option 1:
    ...
    - $\hat{V}_\phi^\pi(s_{t})=\sum\limits_{t=t_0}^T\gamma^{t-t_0}r(s_t,a_t)=r(s_t,a_t)+\gamma\hat{V}_\phi^\pi(s_{t+1})$
  ...
  - Option 1 is we frequently use, while option 2 is **the right solution for** dealing  with the MDP where you actually have probability $1-\gamma$ to die at every state.
...
- **Online** actor-critic algorithm:
  ...
  - <img src="D:\Workspace\MyCheatSheets\Learn\UCB\pic\image-20210227180011842.png" alt="image-20210227180011842" style="zoom:50%;" />
...
- Batch-size:
  ...
  - The state/action pair in batches shouldn't be too correlated. So a better choice for online RL is to use *parallel workers*:
    - <img src="D:\Workspace\MyCheatSheets\Learn\UCB\pic\image-20210228084858019.png" alt="image-20210228084858019" style="zoom:50%;" />
...
- Action-dependent baselines:
  ...
  - ![image-20210228090737613](D:\Workspace\MyCheatSheets\Learn\UCB\pic\image-20210228090737613.png)
...
- Eligibility traces & n-step returns:
  ...
  - We want to combine these two, and use $\hat{A}^\pi_{MC}$ in the short term and $\hat{A}^\pi_{C}$ in the long term:
    ...
    - So one way is to **cut off the tails** in $\hat{A}^\pi_{MC}$, so we won't be affected by the high variance caused by the infinite future, and use critic $\gamma^n\bar{V}_\phi^\pi(s_{t+n})$  to **compensate** for it. 
...
- GAE:
  ...
  - ![image-20210228092953108](D:\Workspace\MyCheatSheets\Learn\UCB\pic\image-20210228092953108.png)
