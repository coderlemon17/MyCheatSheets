...
- **Policy iteration**: = policy evaluation + policy improvement
  - <img src=".\pic\image-20210302114602208.png" alt="image-20210302114602208" style="zoom: 33%;" />
...
- Evaluation of $A^\pi(s,a)$ in Policy Iteration:
  - **Dynamic Programming:**
    ...
    - E.g. <img src=".\pic\image-20210302115459207.png" alt="image-20210302115459207" style="zoom:67%;" />
  - Simpler DP with **Q-function**:
    ...
    - **Value Iteration**:
      -  <img src=".\pic\image-20210302120832554.png" alt="image-20210302120832554" style="zoom: 50%;" />
      - `Remark`: 
...
- Fitted value iteration: "value iteration with function approximator" <font color="red">$\Leftarrow$ Requre to know the transition dynamics! $E(V(s'))$</font>
  - <img src=".\pic\image-20210302162845682.png" alt="image-20210302162845682" style="zoom:50%;" />$\Leftarrow$ Supervised learning for $V_\phi$
  ...
  - **Fitted $Q$ iteration algorithm**:
    - <img src=".\pic\image-20210302164929088.png" alt="image-20210302164929088" style="zoom: 50%;" />
    ...
    - General form:
      - <img src=".\pic\image-20210302165507692.png" alt="image-20210302165507692" style="zoom:50%;" />

## 7.3. From Q-iteration to Q-learning

- What's fitted Q-iteration optimizing:
  - <img src=".\pic\image-20210302194817620.png" alt="image-20210302194817620" style="zoom:50%;" />
- Online Q-learning algorithms:
  - <img src=".\pic\image-20210302194958508.png" alt="image-20210302194958508" style="zoom: 50%;" /> $\Leftarrow$ policy is $\arg\max$, which is <font color="red">*greedy*</font>.
...
- Bellman Operator $\mathcal B$:
  - <img src=".\pic\image-20210303093303199.png" alt="image-20210303093303199" style="zoom:50%;" />
  - `NOTE`:
  ...
  - Convergence:
    - <img src=".\pic\image-20210303093621762.png" alt="image-20210303093621762" style="zoom: 50%;" />
- **Non-tabular** value function learning:
  - <img src=".\pic\image-20210303094016131.png" alt="image-20210303094016131" style="zoom:50%;" />
  ...
  - The general procedure is: $V\rightarrow \mathcal BV\rightarrow$ find the nearest $V'\in\Omega$ to $\mathcal BV$. (projection).
    - <img src=".\pic\image-20210303094240440.png" alt="image-20210303094240440" style="zoom:33%;" />
  ...
  - `Remark`:
    - <img src=".\pic\image-20210303094600945.png" alt="image-20210303094600945" style="zoom:50%;" />
  ...
  - Fitted **Q**-iteration suffers the same limitation: no general convergence guarantee.
    - Q-learning is <font color="red">**NOT**</font> gradient descent of any well-defined objective:
      ...
      - <img src=".\pic\image-20210303095524132.png" alt="image-20210303095524132" style="zoom: 50%;" />
- The fitted **bootstrapped policy evaluation** in batch AC doesn't generally converge:
  - <img src=".\pic\image-20210303101931872.png" alt="image-20210303101931872" style="zoom:50%;" />
...
<img src=".\pic\v2-e7e49525a3eeb7a76355f5685f410d22_1440w.jpg" alt="img" style="zoom:50%;" />

<img src=".\pic\v2-122ce606175d5b35afb9238d96ba0f8d_1440w.jpg" alt="img" style="zoom:50%;" />

# Lecture 08 Deep RL with Q-Functions

## 8.1. Correlated samples in online Q-learning

- Problem: ![image-20210306164027585](.\pic\image-20210306164027585.png)
...
- **Relay buffers**:
  - <img src=".\pic\image-20210306165024361.png" alt="image-20210306165024361" style="zoom:80%;" />
...
- Q-learning with replay buffer and target networks:
  - <img src=".\pic\image-20210306165828500.png" alt="image-20210306165828500" style="zoom:67%;" />
...
<img src=".\pic\image-20210306172444177.png" alt="image-20210306172444177" style="zoom:67%;" />
...
- Overestimation in Q-learning
  ...
  - <img src=".\pic\image-20210308201226624.png" alt="image-20210308201226624" style="zoom: 50%;" />
    ...
    - Luckily, we already have double networks in Q-learning, so we don't need to introduce another network, instead we can use $\phi$ and $\phi'$ for value estimation and action selection separately:
      - <img src=".\pic\image-20210308201704056.png" alt="image-20210308201704056" style="zoom:50%;" />; but the decorrelation is definitely not always perfect, because $\phi\approx\phi'$ sometimes. 
- **Multi-step returns**:
  ...
  - **Multi-step target/N-step return estimator:** <img src=".\pic\image-20210308202640816.png" alt="image-20210308202640816" style="zoom:50%;" />
...
- Problem with continuous action: $\arg\max$
  - <img src=".\pic\image-20210308203848003.png" alt="image-20210308203848003" style="zoom:40%;" />
- Potential solutions:
  - Stochastic optimization: 
    - <img src=".\pic\image-20210308204015847.png" alt="image-20210308204015847" style="zoom: 33%;" />
  ...
  - Use function class that is easy to optimize: e.g. *function class with closed form solutions for their optimal answers.* 
    - Quadratic functions. e.g. Normalized Advantage Functions:
      - <img src=".\pic\image-20210308204442731.png" alt="image-20210308204442731" style="zoom: 33%;" />
  ...
  - Learn an approximate maximizer:
    ...
    - DDPG:
      - <img src=".\pic\image-20210308205139426.png" alt="image-20210308205139426" style="zoom:33%;" /> 
...
- Simple practical tips for Q-learning:
  ...
  - Bellman error gradients can be big; clip gradients or use Huber loss <img src=".\pic\image-20210308205634889.png" alt="image-20210308205634889" style="zoom:25%;" />.
...
- **Policy gradient** as **policy iteration**:
  - We want to prove: $J(\theta')-J(\theta)=E_{\tau\sim p_{\theta'}(\tau)}[\sum\limits_t\gamma^tA^{\pi_\theta}(s_t,a_t)]$.
    ...
    - If we can prove the *right hand* equals to the *left hand*, we can prove that `maximizing the expected value of the advantage of the old policy w.r.t. the trajectories of a new policy` is actually maximizing the RL objective. $\rightarrow$ Policy iteration [|=|](Policy iteration aims to maximize the sum of advantage function by tuning θ') does the right thing.
  - Proof:
    - <img src=".\pic\image-20210309194458112.png" alt="image-20210309194458112" style="zoom: 50%;" /> 
    - `Remark`
  ...
  - The connection between Policy iteration ($E_{\tau\sim p_{\theta'}(\tau)}[\sum\limits_t\gamma^tA^{\pi_\theta}(s_t,a_t)]$) and Policy gradients:
    - <img src=".\pic\image-20210309195640935.png" alt="image-20210309195640935" style="zoom: 67%;" />
...
- Claim: $p_\theta(s_t)$ is close to $p_{\theta'}(s_t)$ when $\pi_\theta$ is close to $\pi_{\theta'}$:
  ...
  - <img src=".\pic\image-20210316171120345.png" alt="image-20210316171120345" style="zoom:40%;" /> 
...
- Implementation:
  - <img src=".\pic\image-20210316174254012.png" alt="image-20210316174254012" style="zoom:50%;" />
...
- Using linear approximation (first order Taylor) of the objective within the trust region:
  - <img src=".\pic\image-20210316184202221.png" alt="image-20210316184202221" style="zoom:50%;" />
    - <font color="grey">And $\nabla_\theta\bar A(\theta)$ is just the normal PG</font>: ![image-20210316184644131](.\pic\image-20210316184644131.png)
- Updating $\theta$ directly using PG $\theta\leftarrow\theta+\alpha \nabla_\theta J(\theta)$ will usually **NOT** satisfied the KL-divergence constraints [|=|](because some small change in some parameters will affect the policy dramatically.)
  - Gradient descent/ascent is actually solving: <img src=".\pic\image-20210316185622869.png" alt="image-20210316185622869" style="zoom:50%;" /> [|=|](Image taking a step to some direction with certain radius constrained by ε.)
    - And the closed form solution for this <img src=".\pic\image-20210316190541075.png" alt="image-20210316190541075" style="zoom:50%;" />
...
- To impose constraints on the *policy space*:
  - We use a second order Taylor expansion of the KL-divergence:
    - <img src=".\pic\image-20210316191211910.png" alt="image-20210316191211910" style="zoom:50%;" />
...
- `Notes and implement tricks`:
  - <img src=".\pic\image-20210316192647554.png" alt="image-20210316192647554" style="zoom:67%;" />
...
- How to make better decisions if we've known the transition dynamics (*optimal control/ trajectory optimization/ planning*):
  - Planning: select a *sequence of actions* [|=|](And thus there's no such thing as policy.) to maximize a utility function s.t. future states are determined by the past.
    - E.g. in the deterministic setting: <img src=".\pic\image-20210318132201122.png" alt="image-20210318132201122" style="zoom:30%;" />
    - E.g. in the stochastic setting: <img src=".\pic\image-20210318132503741.png" alt="image-20210318132503741" style="zoom: 33%;" />
      - `Note`: this *open-loop planning* might not always be the optimal solution.
  - **Loop:**
    - <img src=".\pic\image-20210318132811352.png" alt="image-20210318132811352" style="zoom:30%;" />

## 10.2. Open-loop Planning and MCTS

- Stochastic optimization (black-box optimization)

	> Abstraction: <img src=".\pic\image-20210318133448510.png" alt="image-20210318133448510" style="zoom: 33%;" />
	>
	> Guess && Check: <img src=".\pic\image-20210318134357069.png" alt="image-20210318134357069" style="zoom:33%;" />
	...
	- Cross-entropy method (**CEM**): <font color="grey">sample around the potential best.</font>
	  ...
	  - <img src=".\pic\image-20210318134834376.png" alt="image-20210318134834376" style="zoom:50%;" /> 
	  - `Note`:
...
- Monte Carlo Tree Search:
  ...
  - <img src=".\pic\image-20210318140201812.png" alt="image-20210318140201812" style="zoom:50%;" /> 
  - <img src=".\pic\image-20210318140227660.png" alt="image-20210318140227660" style="zoom:50%;" /> 
  - `Note`:
