...
- <font color='red' size='4'>So linear regression actually has limitation on the dataset : noise needs to be Gaussion and i.i.d</font>
...
- Previous methods are all **discriminative learning algorithms**, this lecture we are going to talk about **generative learning algorithms**. (GDA)
...
- *The email spam problem.* (Naive Bayes is **generative method**.)
...
- **How to make kernels?**
  ...
  - Let $\{x^1,\cdots, x^d\}$ be points, let "kernel matrix" $k\in\mathbb{R}^{d\times d}$, where $k_{ij}=k(x^i,x^j)$.
    ...
    - <img src=".\pic\image-20200916174355486.png" alt="image-20200916174355486" style="zoom:50%;" />
...
## 9.7. My deduction of bias-variance tradeoff

- ![my](/home/lemon/Workspace/myCheatSheet/Learn/Stanford/pic/QQMail_0_1.png)
...
## 10.4. Boosting (Adaboost, xgboost)
...
- Adaboost: 
  ...
  - <font color="grey">Adaboost的本意是在下一次训练的时候强调上一次被分类错误的样本点. 但是奇怪的是, 如果error rate $\geq$ 0.5的时候, 被错误分类的点的权重反而会降低. 一个可能的解释是Adaboosting是假设模型从一个reasonably good的weak model开始训练的. 而这里为什么是0.5可能是因为它假设了这里是一个<font color="red">二分类问题</font>, 所以只要比random好一点点就可.</font>
...
- Gradient Boosting
  ...
  - <img src=".\pic\image-20200929032212916.png" alt="image-20200929032212916" style="zoom: 67%;" />
...
- If we want to classify **three** categories:
  - <img src="./pic/image-20201005182340394.png" alt="image-20201005182340394" style="zoom:67%;" />
...
<img src="./pic/image-20201005190350440.png" alt="image-20201005190350440" style="zoom: 23%;" />
...
- Improving your NNs:
  ...
  - Vanishing and Exploding gradients:
    - Depends on the value of the parameters: 
      - <img src="./pic/cs229_ML_2018.md" alt="image-20201007201936140" style="zoom: 25%;" />
...
<img src="./pic/image-20201010174201943.png" alt="image-20201010174201943" style="zoom: 67%;" />

- The place where the problem happens might **shift**.

## 13.3. Error Analysis

- <img src="./pic/image-20201010174639769.png" alt="image-20201010174639769" style="zoom:80%;" />
...
- Intuition: to deal with the situation where real data only lies in the **subspace** of data space.
...
- **The Factor analysis model**: 
  ...
  - <img src="./pic/image-20201014201008593.png" alt="image-20201014201008593" style="zoom: 33%;" />
    ...
    - This can be achieved through assume: <img src="./pic/image-20201014201235516.png" alt="image-20201014201235516" style="zoom:25%;" />
...
- **E-step**: $Q_i(z^i)=p(z^i|x^i;\mu,\Lambda,\Psi)$:
  ...
  - <img src="./pic/image-20201014211253878.png" alt="image-20201014211253878" style="zoom: 33%;" />
...
- How to choose axis $u$: where the covariance of projected data is maximized, because $x$ is normalized, $x^Tu$ will still have **zero-mean**:
  -  For example, if we want to find a 1-dimensional subspace to approximate the data, we need to maximize: $\max\frac{1}{n}\sum\limits_{i=1}^n((x^i)^Tu)^2=\max\frac{1}{n}\sum\limits_{i=1}^n(u^Tx^i(x^i)^Tu)=\max u^T\left(\frac{1}{n}\sum\limits_{i=1}^nx^i(x^i)^T\right)u$.
  - Let $\Sigma=\sum\limits_{i=1}^nx^i(x^i)^T$, which is the empirical covariance matrix of the data (with zero mean). 
    -  **NOTE that $x^i$** is sample $i$, **NOT** dimension $i$.
    -  **Covariance**: $Cov(X,Y)=E[(X-\mu_x)(Y-\mu_y)]$; Covariance matrix: $E[(X-\mu_x)(X-\mu_x)^T]$; correlation $\rho=\frac{Cov(X,Y)}{\sigma_X\sigma_Y}$.
  - So the problem is: $\min\limits_u -u^T\Sigma u\ s.t.\ u^Tu=1$:
    -  Using Lagrange multiplier: $\Sigma u=\lambda u$, so $u$ is the eigenvector of $\Sigma$, and because $\Sigma$ is symmetric, the $u_i$ will (or always can be chosen to be) orthogonal to each other.
    -  `Remark`: $\nabla_u u^T\Sigma u$想象成$\nabla_u u^2 = 2u$.
- **PCA**: <img src="./pic/image-20201025201853127.png" alt="image-20201025201853127" style="zoom: 33%;" />
...
- Problem:
  ...
  -  $s_i^t$ is the sound of speaker $i$ at time $t$, and the recorded sound is $x^t=As^t$, where $s,x\in\mathbb{R}^n$ (we have $n$ speakers and $n$ microphones). And the goal is to find $W=\begin{bmatrix}W_1^T\\\vdots\\W_n^T\end{bmatrix}\overbrace{=}^{\text{hopefully}}A^{-1}$ to recover the voice of each speaker. Here the unknown matrix $A$ is called *the mixing matrix*, and $W$ is *inmixing matrix*.
- **ICA ambiguities**:
  - Permutation matrix: e.g. <img src="./pic/image-20201025203327408.png" alt="image-20201025203327408" style="zoom:20%;" />, we cannot distinguish between $W$ and $PW$ only given $x^i$s. (It's like switch the position of the microphones.)
  ...
  - Remark: <img src="./pic/image-20201025204221840.png" alt="image-20201025204221840" style="zoom:33%;" />
...
- **Density and linear transformations**
  ...
  - **NOT EQUAL TO 重积分换元**: [link](https://zhuanlan.zhihu.com/p/171701301): 
      - <img src=".\pic\image-20210228220653401.png" alt="image-20210228220653401" style="zoom: 67%;" />
  - One interpretation: 
    - Note that: the determinant is the scaling factor of linear transformation.
      - So let $C_1=[0,1]^d$ denote the $d$-dimensional hypercube. And $A\in\mathbb R^{d\times d}$ actually maps $C_1$ to a set of volume $|A|$.
      - Define $C_2=\{As:s\in C_1\}\subseteq\mathbb R^{d}$ to be the image of $C_1$ under mapping $A$, which has a volume of $|A|$.
      - Suppose $s$ is uniformly distributed in $C_1$, which means $p_s(s)=1\{s\in C_1\}$. Then clearly $x$ will be uniformly distributed in $C_2$, and $p_x(x)=1\{x\in C_2\}/vol(C_2)$.
      - By using $\frac{1}{|A|}=|A^{-1}|$, we can get $p_x(x)=1\{x\in C_2\}|A^{-1}|=1\{A^{-1}x\in C_1\}|A^{-1}|=p_s(A^{-1}x)|A^{-1}|$.
...
- **ICA algorithm**:
  ...
  - Use gradient descent for updating $W$: 
    - <img src="./pic/image-20201025212051531.png" alt="image-20201025212051531" style="zoom:50%;" />
  - Remark:
    - ![image-20201025212142747](./pic/image-20201025212142747.png)
...
- $<S,A,\{P_{sa}\},\gamma,R>$.
  - At $s_t$ choose $a_t$, enter $s_{t+1}$ and receive $r_t$. (receive reward as you enter one state)
...
- Bellman Equation for $V_{\pi}$: <img src=".\pic\image-20201030112658154.png" alt="image-20201030112658154" style="zoom:67%;" />
  - For $V^*$: $V_{\pi^*}=\max\limits_{a}\sum\limits_{s'}\mathcal{P}^{a}_{ss'}[\mathcal{R}^a_{ss'}+\gamma V_{\pi^*}(s')]$
