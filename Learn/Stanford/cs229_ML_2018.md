...
- <img src="/home/lemon/Workspace/myCheatSheet/Learn/Stanford/pic/image-20200928220438864.png" alt="image-20200928220438864" style="zoom:25%;" />
  - We accomplish this by petitioning the input space $X$ into disjoint subsets (or **regions**) $R_i$.
- How to selecting $Regions$:
  - **Top-down**, leaf node (feature dimension), threshold.
  - Loss function:
    ...
    - And the loss of one region has multiple choices: (Let $\hat{p}_c$ denote the proportion of examples in $R$ that are of class $c$):
      - $L_{misclass}(R)=1-\max\limits_c(\hat{p}_c)$, which is the number of examples that would be misclassified if we predicted the majority class for region $R$.
  - 
