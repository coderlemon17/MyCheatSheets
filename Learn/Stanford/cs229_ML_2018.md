...
- How to selecting $Regions$:
  ...
  - Loss function:
    ...
    - And the loss of one region has multiple choices:
      - Let $\hat{p}_c$ denote the proportion of examples in $R$ that are of class $c$), $L_{misclass}(R)=1-\max\limits_c(\hat{p}_c)$, which is the number of examples that would be misclassified if we predicted the majority class for region $R$. 
      - Cross-entropy Loss: 
  - 
